#include <PnC/LaikagoPnC/TaskSet/TaskSet.hpp>
...
bool CoMxyzRxRyRzTask::_UpdateCommand(const Eigen::VectorXd & _pos_des,
    ...
    Eigen::Quaternion<double> ori_act(robot_->getBodyNodeCoMIsometry(LaikagoBodyNode::trunk).linear());
...
bool CoMxyzRxRyRzTask::_UpdateTaskJacobian(){
    Eigen::MatrixXd Jtmp = robot_->getBodyNodeCoMJacobian(LaikagoBodyNode::trunk);
    // (Rx, Ry, Rz)

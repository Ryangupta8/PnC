#include <PnC/A1PnC/ContactSet/ContactSet.hpp>
#include <PnC/A1PnC/CtrlSet/CtrlSet.hpp>
#include <PnC/A1PnC/TaskSet/TaskSet.hpp>
...
BalanceCtrl::BalanceCtrl(RobotSystem* robot) : Controller(robot) {
    ...
    Kp_ = Eigen::VectorXd::Zero(A1::n_adof);
    Kd_ = Eigen::VectorXd::Zero(A1::n_adof);
    ...
    total_joint_task_ =
        new BasicTask(robot, BasicTaskType::JOINT, A1::n_adof);
    ...
    //Line Contact
    frfoot_contact_ = new PointContactSpec(robot_, A1BodyNode::FR_foot, 0.7);    
    flfoot_contact_ = new PointContactSpec(robot_, A1BodyNode::FL_foot, 0.7);    
    rlfoot_contact_ = new PointContactSpec(robot_, A1BodyNode::RL_foot, 0.7);    
    rrfoot_contact_ = new PointContactSpec(robot_, A1BodyNode::RR_foot, 0.7);    

    //dim_contact_ = rfoot_contact_->getDim() + lfoot_contact_->getDim();
    dim_contact_ = frfoot_contact_->getDim() + flfoot_contact_->getDim() +
                    rlfoot_contact_->getDim() + rrfoot_contact_->getDim();

    
    wblc_data_->W_qddot_ = Eigen::VectorXd::Constant(A1::n_dof, 100.0);
    wblc_data_->W_rf_ = Eigen::VectorXd::Constant(dim_contact_, 0.1);
    wblc_data_->W_xddot_ = Eigen::VectorXd::Constant(dim_contact_, 1000.0);
    wblc_data_->W_rf_[frfoot_contact_->getFzIndex()] = 0.01;
    wblc_data_->W_rf_[frfoot_contact_->getDim() + rfoot_back_contact_->getFzIndex()] =
        0.01;
    wblc_data_->W_rf_[frfoot_contact_->getDim() + rfoot_back_contact_->getDim() +
    ...
    wblc_data_->tau_min_ = Eigen::VectorXd::Constant(A1::n_adof, -2500.);
    wblc_data_->tau_max_ = Eigen::VectorXd::Constant(A1::n_adof, 2500.);

    sp_ = A1StateProvider::getStateProvider(robot);
...
void BalanceCtrl::oneStep(void* _cmd) {
    ...
    Eigen::VectorXd gamma = Eigen::VectorXd::Zero(A1::n_adof);
    _contact_setup();
    _task_setup();
    _compute_torque_wblc(gamma); 
    for (int i(0); i < A1::n_adof; ++i) {
        ((A1Command*)_cmd)->jtrq[i] = gamma[i];
...
void BalanceCtrl::_compute_torque_wblc(Eigen::VectorXd& gamma) {
    ...
    Eigen::VectorXd des_jacc_cmd =
        ...
        Kp_.cwiseProduct(des_jpos_ -
                         sp_->q.segment(A1::n_vdof, A1::n_adof)) +
        Kd_.cwiseProduct(des_jvel_ - sp_->qdot.tail(A1::n_adof));
...
void BalanceCtrl::_task_setup() {
    ...
    Eigen::VectorXd jvel_des(A1::n_adof);
    jvel_des.setZero();
    Eigen::VectorXd jacc_des(A1::n_adof);
    jacc_des.setZero();
...
void BalanceCtrl::_contact_setup() {
    ...
    flfoot_contact_->updateContactSpec();
    frfoot_contact_->updateContactSpec();
    rlfoot_contact_->updateContactSpec();
    rrfoot_contact_->updateContactSpec();
...
void BalanceCtrl::firstVisit() {
    jpos_ini_ = sp_->q.segment(A1::n_vdof, A1::n_adof);
